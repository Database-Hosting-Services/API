...
const (
	...
	RetrieveUserProjects = `SELECT id, oid, owner_id, name, description, status, created_at, api_url, api_key
							FROM "projects"
							WHERE owner_id = $1`

	RetrieveUserSpecificProject = `SELECT id, oid, owner_id, name, description, status, created_at, api_url, api_key
									FROM "projects"
